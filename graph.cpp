...
void graph::loadgraph(string path,int bound)
{
    ...
    edgeCount*=2;
    int n=uCount+vCount;
    int par_num=n%bound==0?n/bound:n/bound+1;
    ...
    fstream parbeginFile(path+"/partitionbegin.bin", ios::in|ios::binary);
    fstream paradjFile(path+"/partitionadj.bin", ios::in|ios::binary);
    ...
    breakVertex32=lower_bound(deg,deg+n,bound,cmp1)-deg;
    vertexCount=uCount+vCount;
    delete(deg);
}

void graph::loadgraph_test(string path,int bound)
{
    //printf( "Loading graph...\n" );
    fstream propertiesFile(path+"/properties.txt", ios::in);
    propertiesFile>>uCount>>vCount>>edgeCount;
    ...
    int par_num=n%bound==0?n/bound:n/bound+1;
    beginPos = new long long[uCount+vCount+1];
    par_beginPos=new long long[(uCount+vCount)*par_num+1];
    
    edgeList = new int[edgeCount];
    par_edgeList=new int[edgeCount];
    propertiesFile.close();    
    fstream beginFile(path+"/begin.bin", ios::in|ios::binary);
    fstream adjFile(path+"/adj.bin", ios::in|ios::binary);
    fstream partitionbeginFile(path+"/partitionbegin.bin", ios::in|ios::binary);
    fstream partitionadjFile(path+"/partitionadj.bin", ios::in|ios::binary);
    beginFile.read((char*)beginPos,sizeof(long long)*(uCount+vCount+1));
    adjFile.read((char*)edgeList,sizeof(int)*(edgeCount));
    partitionbeginFile.read((char*)par_beginPos,sizeof(long long)*(uCount+vCount)*par_num+1);
    partitionadjFile.read((char*)par_edgeList,sizeof(int)*(edgeCount));
    beginFile.close();
    adjFile.close();
    partitionadjFile.close();
    partitionbeginFile.close();
    int* deg = new int[uCount+vCount];
    for( int i = 0; i < uCount+vCount; ++i ) 
        deg[i]=beginPos[i+1]-beginPos[i];
    breakVertex10=lower_bound(deg,deg+n,10,cmp1)-deg;
...
void graph::loadWangkaiGraph(string path,int bound)
{
